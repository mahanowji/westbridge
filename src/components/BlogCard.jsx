...
export default function BlogCard({ post }) {
  ...
  return (
    <Box
      component="article"
      sx={{ borderTop: '1px solid', borderColor: 'divider', pt: 2, display: 'flex', flexDirection: 'column', gap: 0.5 }}
    >
      ...
      <Typography variant="h3" sx={{ mb: 0.2 }}>
        <Box
          component={Link}
          to={`/blog/${post.slug}`}
          sx={{ color: 'inherit', textDecoration: 'none', '&:hover': { color: 'eucalyptus.dark' } }}
        >
          {post.title[lang]}
        </Box>
      </Typography>
      <Typography sx={{ color: 'text.secondary', mb: 0.5 }}>{post.excerpt[lang]}</Typography>
      <Box
        component={Link}
        to={`/blog/${post.slug}`}
        sx={{ color: 'brass.dark', fontWeight: 600, fontSize: '0.82rem', textDecoration: 'none', '&:hover': { textDecoration: 'underline' } }}
      >
        {t('blog.readMore')}
      </Box>
    </Box>
